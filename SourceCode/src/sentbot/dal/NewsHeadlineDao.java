...
public class NewsHeadlineDao extends SourceDao {
	...
	public List<NewsHeadline> getNewsHeadlineBySector(Sector sector) throws SQLException {
		List<NewsHeadline> newsHeadline = new ArrayList<NewsHeadline>();
		String selectNewsHeadlineByFactors = 
			"SELECT nh.SourceKey AS SourceKey, Sentiment, Sector, NewsSource, Headline "
			+ "FROM NewsHeadline AS nh INNER JOIN Source AS s ON nh.SourceKey = s.SourceKey "
		...
		public NewsHeadline updateHeadline(NewsHeadline newsHeadline, String newHeadline) throws SQLException {
		String updateNewHeadline = "UPDATE NewsHeadline SET Headline=? WHERE SourceKey=?;";
		Connection connection = null;
		PreparedStatement updateStmt = null;
		try {
			connection = connectionManager.getConnection();
			updateStmt = connection.prepareStatement(updateNewHeadline);
			updateStmt.setString(1, newHeadline);
			updateStmt.setInt(2, newsHeadline.getSourceKey());
			updateStmt.executeLargeUpdate();
			newsHeadline.setHeadline(newHeadline);
			return newsHeadline;
		} catch (SQLException e) {
			e.printStackTrace();
			throw e;
	...
	//Update NewsHeadline Sentiment
	public NewsHeadline updateSentiment(NewsHeadline newsHeadline, String newSentiment) throws SQLException {
		Source s = new Source(newsHeadline.getSourceKey(), newsHeadline.getSentiment(), newsHeadline.getSector());
		super.updateSentiment(s, newSentiment);
		newsHeadline.setSentiment(newSentiment);
		return newsHeadline;
	}
